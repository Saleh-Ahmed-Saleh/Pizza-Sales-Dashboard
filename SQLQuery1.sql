...
SELECT pizza_category,
CAST(SUM(total_price) AS decimal(10,2)) AS Total_revenue,
CAST(SUM(total_price) * 100 / (SELECT SUM(total_price) from pizza_sales$ WHERE MONTH(order_date) = 9) AS DECIMAL(10,2)) AS PCT
FROM pizza_sales$
...
SELECT pizza_category,
CAST(SUM(total_price) AS decimal(10,2)) AS Total_revenue,
CAST(SUM(total_price) * 100 / (SELECT SUM(total_price) from pizza_sales$ WHERE DATEPART(QUARTER,order_date) = 3) AS DECIMAL(10,2)) AS PCT
FROM pizza_sales$
